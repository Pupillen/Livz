...
const std::string Livz::version_ = "BETA 1.0";
...
void Livz::init(const ros::NodeHandle& nh) {
    ...
    getInstance().tick_timer    = nh.createTimer(ros::Duration(0.01),   [&](const ros::TimerEvent& event) { return getInstance().LivzTick(event); });
    getInstance().cu_tick_timer = nh.createTimer(ros::Duration(0.01),   [&](const ros::TimerEvent& event) { return getInstance().LivzCusUpdaterTick(event); });
}
